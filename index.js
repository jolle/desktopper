...
module.exports = (rootUrl) => {
    ...
    const icon = axios.get(rootUrl, {
        ...
        .then(({ data }) => cheerio.load(data)) // eslint-disable-line
    ...
    const downloader = icon.then(iconUrl => axios.get(iconUrl, { responseType: 'arraybuffer' }))
        .then(({ data }) => fs.writeFileSync('temp/icon.png', data));

    const converter = downloader.then(() => process.platform === 'darwin' ? png2icns({ in: 'temp/icon.png', out: 'temp/icon.icns' }) : Promise.promisify(pngToIco)('temp/icon.png').then((buf) => fs.writeFileSync('temp/icon.ico'))); // eslint-disable-line
